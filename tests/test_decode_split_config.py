...
class DecodeSplitConfigTest(unittest.TestCase):
    ...
    def test_h20_primary_shape_auto_config(self):
        ...
        self.assertEqual(config.target_ctas_per_sm, 2)
        self.assertEqual(config.target_grid_blocks, 156)
        self.assertEqual(config.num_blocks, 2048)
        self.assertEqual(config.q_groups, 1)
        self.assertEqual(config.split_fanout, 8)
        self.assertEqual(config.auto_num_splits, 19)
        self.assertIsNone(config.requested_num_splits)
        self.assertEqual(config.num_splits, 19)
        self.assertEqual(config.actual_grid_blocks, 152)
        self.assertAlmostEqual(config.estimated_waves, 152 / 156)
        self.assertEqual(config.base_blocks_per_split, 107)
        self.assertEqual(config.long_splits, 15)
        self.assertEqual(config.min_split_blocks, 107)
        self.assertEqual(config.max_split_blocks, 108)
        self.assertEqual((config.num_splits, 8, 1), (19, 8, 1))

    def test_h20_primary_shape_distribution_and_coverage(self):
        split_counts = self.assert_complete_coverage(self.h20_config())

        self.assertEqual(split_counts[:15], [108] * 15)
        self.assertEqual(split_counts[15:], [107] * 4)
    ...
    def test_twenty_split_override(self):
        ...
        self.assertAlmostEqual(config.estimated_waves, 160 / 156)
    ...
    def test_q_groups_increase_split_fanout(self):
        ...
        self.assertEqual(config.num_splits, 9)
        self.assert_complete_coverage(config)
    ...
    def test_diagnostic_format(self):
        ...
        for expected in (
            ...
            "target_grid_blocks=156",
            "split_fanout=8",
            "num_splits=19",
            "actual_grid_blocks=152",
            "estimated_waves=0.974359",
            "base_blocks_per_split=107",
            "long_splits=15",
            "min_split_blocks=107",
            "max_split_blocks=108",
        ):
