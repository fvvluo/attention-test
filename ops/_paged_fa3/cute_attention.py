    ...
    if os.environ.get("_NO_FMHA_REF", "0") == "0" and Sq >= 128:
        try:
            from paged_fa3._fmha_ref.adapter import (
                ...
                can_use_fp8_prefill, fp8_hybrid_attention)
            # FP8 hybrid: ~1.9x the bf16 roofline on long causal prefill. Bulk in
            # e4m3, first NCORR causal rows recomputed in bf16 for accuracy. Gated
