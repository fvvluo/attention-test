# ============================================================
# 融合算子 v24：按 q_len 路由 prefill / decode，得到完整 FlashAttention
...
# 只做"拼接/融合"，不修改任何已有文件
#   （_prefill9.py / _prefill9_kernel.py / _prefill9_helpers.py / _decode13.py 只读）。
# 全程 BHSD：
#   q:  (b, q_heads,  q_len,  d)
#   k/v:(b, kv_heads, kv_len, d)
#   out 与 q 同 shape。q_heads 是 kv_heads 的整数倍（GQA）；MHA 时相等。
#
# 组合说明：prefill 用 v9（_prefill9），decode 用 v13（fused21 的最优 decode）。
...
# ------------------------------------------------------------
# decode 驱动（q_len == 1）：调用 _decode13.py，固定生产配置（不走 autotune）。
# ------------------------------------------------------------
...
register("prefill_decode_fused24 (cute+cute)", attention)
