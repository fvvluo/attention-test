...
_COMPILED_CACHE = {}
# num_splits="auto" fast path: (device, q.shape, k.shape, v.shape, sm_scale,
# use_pdl, use_fused) -> (compiled, o_part, lse_part, counter, device). Lets
# steady-state auto calls bypass autotune lookup + config unpack + key build.
_FASTPATH_CACHE = {}
...
_AUTO_CFG_CACHE = {}
# Each candidate: (num_splits, num_stages, block_n, num_workers). num_workers=None
# uses the default (num_splits*kv_heads*batch = one CTA per work-item).
_AUTO_CFG_CANDIDATES = (
    # BLOCK_N=256 family dominates (tile count halved -> softmax barriers halved).
    # (9,1,256,78) is the proven best on every fast instance tested; it is FIRST
    # so it is the default that near-tie challengers must beat by a margin.
    (9, 1, 256, 78),   # persistent grid = 78 SMs, 1 wave, no tail. ~3.64 TB/s.
    (9, 1, 256, 84),
    (8, 1, 256, 78),
    (16, 1, 256, 128),
    # bn=128 fallbacks for instances/shapes where bn=256 is not divisible or slower
    (6, 3, 128, None),
    (32, 3, 128, None),
    (48, 3, 128, None),
)
# A challenger must be at least this much faster than the incumbent (the earlier,
# more-proven candidate) to be selected — makes autotune robust to micro-benchmark
# noise/contention so it doesn't lock in a worse config from a noisy first call.
_AUTO_MARGIN = 0.02
...
def _autotune_config(q, k, v, sm_scale, use_pdl, kv_len, kv_heads, batch):
    ...
    def _time(ns, st, bn, nw):
        # robust: generous warmup + best-of-5 rounds to reject contention spikes.
        for _ in range(15):
            attention_decode(q, k, v, sm_scale=sm_scale, num_splits=ns,
                             block_n=bn, num_stages=st, num_workers=nw, use_pdl=use_pdl)
        ...
        for _ in range(5):
            ...
            ev1.record()
            torch.cuda.synchronize()
            ms = min(ms, ev0.elapsed_time(ev1) / 40)
        return ms

    best_cfg = None
    best_ms = float("inf")
    for ns, st, bn, nw in _AUTO_CFG_CANDIDATES:
        if kv_len % bn != 0:
            continue
        try:
            ms = _time(ns, st, bn, nw)
        except Exception:
            continue
        if best_cfg is None:
            best_cfg, best_ms = (ns, st, bn, nw), ms
        # A later candidate only wins if it beats the incumbent by the margin,
        # so near-ties keep the earlier (more-proven) config -> stable choice.
        elif ms < best_ms * (1.0 - _AUTO_MARGIN):
            best_cfg, best_ms = (ns, st, bn, nw), ms
    if best_cfg is None:
...
def attention_decode(
    ...
    # ---- auto fast path -------------------------------------------------
    # Once autotune has selected a config and compiled the kernel for a given
    # (device, shapes, scale, pdl), skip ALL per-call Python work (autotune
    # lookup, config unpack, 15-field key build, _COMPILED_CACHE lookup) and
    # invoke the compiled kernel directly. Removes the ~4us/call auto overhead
    # (measured 3.54 -> 3.64 TB/s). Correctness is unchanged: the fast entry is
    # only populated after a normal (validated) compile below.
    if num_splits == "auto":
        _ss = 1.0 / math.sqrt(q.shape[-1]) if sm_scale is None else float(sm_scale)
        fk = (q.device.index, q.shape, k.shape, v.shape, _ss, use_pdl, use_fused)
        fast = _FASTPATH_CACHE.get(fk)
        if fast is not None:
            compiled, o_part, lse_part, counter, dev = fast
    ...
    _is_auto = num_splits == "auto"
    if _is_auto:
        num_splits, num_stages, block_n, _auto_nw = _autotune_config(
            q, k, v, sm_scale, use_pdl, kv_len, kv_heads, batch
        )
        if _auto_nw is not None:
            num_workers = _auto_nw
    ...
    with torch.cuda.device(q.device):
        ...
        # Register the auto fast path so subsequent same-shape calls skip all
        # per-call config resolution and go straight to the compiled kernel.
        if _is_auto:
            fk = (q.device.index, q.shape, k.shape, v.shape,
                  float(sm_scale), use_pdl, use_fused)
            _FASTPATH_CACHE[fk] = (compiled, o_part, lse_part, counter, q.device.index)
    return o
