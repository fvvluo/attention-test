...
_AUTO_CFG_CANDIDATES = (
    (9, 1, 256, 78),   # WINNER on fast instances: BLOCK_N=256 halves the tile
                       # count -> halves softmax barriers; persistent grid=78 SMs
                       # (72 items, 1 wave, no tail). Measured 3.64 TB/s on GPU5.
    (9, 1, 256, 90),
    (8, 1, 256, 64),
    (16, 1, 256, 128),
    (6, 3, 128, None),  # very long splits: minimal combine + per-item overhead
    (16, 3, 128, None),
    (52, 3, 128, None),
    (48, 3, 128, None),
    (32, 3, 128, None),  # strong on GPU 2/4/7
    (39, 3, 128, None),
    (64, 3, 64, None),   # bn=64 fits 2 CTAs/SM
)
...
def _autotune_config(q, k, v, sm_scale, use_pdl, kv_len, kv_heads, batch):
    ...
    for ns, st, bn, nw in _AUTO_CFG_CANDIDATES:
        ...
        try:
            ...
            if ms < best_ms:
                best_ms, best_cfg = ms, (ns, st, bn, nw)
        except Exception:
            continue
