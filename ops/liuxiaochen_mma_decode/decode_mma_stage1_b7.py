#!/usr/bin/env python3
"""B7: B6 + Programmatic Dependent Launch (PDL) Stage-1.

Identical to B6 (cp.async 2-stage warp-MMA decode) except for PDL: the Stage-1
launch sets use_pdl=True and calls cute.arch.griddepcontrol_launch_dependents()
after the block loop (before the epilogue), so the dependent Stage-2 combine grid
can begin launching while Stage-1 writes its final partials. The combine kernel
(decode_mma_combine_b7) calls griddepcontrol_wait() before reading any producer
data, so PDL overlaps only launch/scheduling latency, never data hazards. All MMA
mapping, cp.async pipeline, softmax math and partial format are unchanged from B6.

Original B6 header follows.
...
class LiuXiaochenMmaDecodeStage1B7:
    ...
    def __call__(self, mQ, mK, mV, mPartial, mLSE, scale_log2, stream: cuda.CUstream):
        ...
        self.kernel(
            ...
            use_pdl=True,  # B7: allow the dependent combine grid to pre-launch.
        )
    ...
    def kernel(self, mQ, mK, mV, mPartial, mLSE, scale_log2,
        ...
        # B7 PDL: signal that dependent (combine) grid may start launching while
        # this CTA writes its final partial. The combine kernel calls
        # griddepcontrol_wait() before reading any producer data, so this only
        # overlaps launch/scheduling latency, never data hazards.
