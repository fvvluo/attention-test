#!/usr/bin/env python3
"""B7 combine: B5 LSE-weighted split combine + PDL consumer wait.

Identical math to the B5 combine (decode_mma_combine.py); the only additions are
PDL: the launch sets use_pdl=True and the kernel calls
cute.arch.griddepcontrol_wait() before reading any producer (partial_o/LSE) data,
so the grid may pre-launch under PDL but never reads before Stage-1 signals
completion via griddepcontrol_launch_dependents().

Independent implementation by Liu Xiaochen. One CTA (128 threads) per query head.
FP32 partials -> BF16 output. Pure GPU, no torch.max/sum, no CPU.
"""
...
class LiuXiaochenMmaDecodeCombineB7:
    ...
    def __call__(self, mPartial, mLSE, mO, stream: cuda.CUstream):
        ...
        self.kernel(mPartial, mLSE, mO, Smem).launch(
            ...
            stream=stream,
            use_pdl=True,  # B7: dependent grid pre-launch under PDL.
        )
    ...
    def kernel(self, mPartial, mLSE, mO, Smem: cutlass.Constexpr):
        ...
        # B7 PDL: wait for the Stage-1 producer to signal completion before
        # reading any partial_o / LSE. Guarantees no read-before-write hazard.
