#!/usr/bin/env python3
"""B7 (cp.async + PDL) layered correctness vs B6 / baseline / SDPA.

Usage: python3 ops/liuxiaochen_mma_decode/verify_mma_b7.py --gpu <idle>
Rule: PASS if max_abs<=2e-2 OR max_rel<=2e-2. B7 must match B6 bit-identically.
"""
...
from runner_b7 import mma_decode_b7 as B7  # noqa: E402
...
def main():
    ...
    for kv, ns, seeds, sdpa in cases:
        for seed in seeds:
            ...
            out7 = B7(q, k, v, sm_scale=sm, num_splits=ns)
            out6 = B6(q, k, v, sm_scale=sm, num_splits=ns)
            print(f"  kv={kv} splits={ns} seed={seed}: {tuple(out7.shape)} {out7.dtype}")
            ok = True
            ok &= cmp("B7 vs B6", out7, out6)
            ok &= cmp("B7 vs baseline", out7, bl(q, k, v, sm))
            if sdpa:
                ok &= cmp("B7 vs SDPA", out7, sdpa_ref(q, k, v, sm))
            all_ok &= ok
            if not ok:
