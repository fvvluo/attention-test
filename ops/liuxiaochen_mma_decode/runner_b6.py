#!/usr/bin/env python3
"""B6 cp.async double-buffered warp-MMA GQA decode runner.
...
Identity: "b6-mma-cpasync-2stage". Independent compile/workspace caches.
"""
...
def _workspace(q, num_splits):
    ...
    key = ("b6", q.device.index, stream_id, q.dtype, tuple(q.shape), num_splits)
...
def mma_decode_b6(q, k, v, sm_scale=None, num_splits=None, n_block=64):
    if q.dim() != 4 or k.dim() != 4 or v.dim() != 4:
        raise ValueError("q/k/v 必须是 BHSD 四维")
    if q.shape[2] != 1:
        raise ValueError("B6 decode 要求 q_len==1")
    if tuple(q.shape) != (1, 64, 1, 128):
        raise ValueError("B6 首版要求 Q shape (1,64,1,128)")
    if tuple(k.shape[:2]) != (1, 8) or k.shape != v.shape or k.shape[-1] != 128:
        raise ValueError("B6 首版要求 K/V shape (1,8,Sk,128)")
    if k.shape[2] % n_block != 0:
        raise ValueError(f"B6 要求 kv_len % {n_block} == 0")
    if not (q.is_cuda and k.is_cuda and v.is_cuda):
        raise ValueError("B6 仅支持 CUDA")
    ...
    if not (q.dtype == torch.bfloat16 and k.dtype == torch.bfloat16 and v.dtype == torch.bfloat16):
        raise ValueError("B6 仅支持 BF16")
    if not (q.is_contiguous() and k.is_contiguous() and v.is_contiguous()):
        raise ValueError("B6 要求 contiguous BHSD 输入")
    major, minor = torch.cuda.get_device_capability(q.device)
    if (major, minor) != (9, 0):
        raise ValueError(f"B6 需要 SM90，当前 SM{major}{minor}")
    ...
    from decode_mma_stage1_b6 import LiuXiaochenMmaDecodeStage1B6
    ...
    key = ("b6-mma-cpasync-2stage", q.device.index, int(torch_stream.cuda_stream), q.dtype,
           tuple(q.shape), tuple(k.shape), n_block, ns, partial.dtype)
    compiled = _compile_cache.get(key)
    if compiled is None:
        s1 = LiuXiaochenMmaDecodeStage1B6(
