...
# Persistent TMA scratch allocator.
#
# Triton asks for device scratch for its device-side TMA descriptors on every
# launch. The stock allocator does a fresh torch.empty each time; that is a new
# allocation *inside* the CUDA-graph capture region, which forces the captured
# graph into a private memory pool whose placement depends on the surrounding
# allocator state (a huge transient allocation right before capture could push
# it into a fragmented region and cost ~8% bandwidth on replay).
#
# We instead hand out slices of a persistent, grow-only buffer keyed by
# (device, stream). Because the buffer already exists, no allocation happens
# during capture, so the graph's TMA scratch address is stable and its replay
# performance is independent of allocator history. The buffer only ever grows.
_TMA_SCRATCH: dict = {}
...
def _tma_allocator(size, alignment, stream):
    ...
    key = (dev, int(stream))
    buf = _TMA_SCRATCH.get(key)
    if buf is None or buf.numel() < size:
        # Round up to reduce re-allocations and keep 16B alignment headroom.
        nbytes = max(size, 1)
        nbytes = (nbytes + 255) & ~255
        buf = torch.empty(nbytes, dtype=torch.int8, device=dev)
        _TMA_SCRATCH[key] = buf
    return buf[:size]
...
class _GraphEntry:
    __slots__ = ("graph", "pacc", "pml", "out", "lse", "p", "t_ref", "calls")
...
def flash_attention_decode(
    ...
    *,
    ...
    """
    # Fast path: if we already have an entry for these exact input buffers,
    # skip the _plan()/key rebuild host work (which otherwise stalls the tiny
    # gap between the two GPU kernels). Replay the graph if we kept one; else
    # relaunch eagerly into the cached persistent buffers (== v2 behaviour).
    ...
    if entry is None:
        ...
        # Warm up on a side stream (PyTorch's recommended graph-capture prelude)
        # so Triton JIT/autotune settles and the persistent TMA scratch is sized
        # BEFORE capture. With the persistent allocator, capture then performs no
        # allocation and the graph's memory placement is deterministic.
        eager_out = None
        try:
            ...
            # Reference output from the (correct) eager warmup, for a post-capture
            # sanity check.
            eager_out = entry.out.clone()

            g = torch.cuda.CUDAGraph()
            with torch.cuda.graph(g):
                _launch()
            # Validate the replay actually reproduces the eager result. Zero the
            # output first so a no-op/misplaced graph is caught.
            entry.out.zero_()
            g.replay()
            torch.cuda.synchronize()
            ok = torch.allclose(entry.out, eager_out, rtol=1e-2, atol=1e-2)

            # Performance guard: a captured graph can land in a fragmented
            # memory pool (if a huge allocation preceded capture) and replay
            # SLOWER than plain eager launches. Measure both and only keep the
            # graph if it is genuinely faster, so v3 is never worse than v2
            # regardless of allocator history at capture time.
            #
            # Robustness: the comparison is done as several INTERLEAVED rounds
            # of best-of-N, so a one-off scheduling hiccup on a shared GPU can't
            # flip the decision. graph and eager are timed back-to-back in each
            # round to cancel out slow drift (clock ramp / neighbour load).
            if ok:
                def _t(run, iters=50):
                ...
                t_eager = float("inf")
                for _ in range(5):
                    t_graph = min(t_graph, _t(g.replay))
                    t_eager = min(t_eager, _t(_launch))
                # Require a real margin (1.5%) to prefer the graph; otherwise
                # keep eager (identical to v2) which is the safe default.
                entry.graph = g if t_graph < t_eager * 0.985 else None
                # Remember the measured replay cost so the runtime watchdog can
                # detect if a kept graph later regresses badly.
                entry.t_ref = t_graph if entry.graph is not None else t_eager
            else:
                entry.graph = None
                entry.t_ref = 0.0
        except Exception:
            # Capture unsupported/failed: fall back to eager for this key.
            entry.graph = None
            _launch()  # ensure entry.out holds a correct result to return

        _GRAPH_CACHE[key] = entry
        if entry.graph is None:
            # Recompute eagerly so the returned buffer is definitely correct
            # (the failed-capture / rejected-graph branch may have left it in a
            # stale/zeroed state from the timing probes).
            _launch()
        # Register the fast-path hit either way: graph -> replay, else -> eager
        # relaunch into the persistent buffers, both skipping _plan() rebuild.
