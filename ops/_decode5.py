...
class FlashDecodeKernel:
        ...
        if warp_idx < 4:
            ...
            for item in cutlass.range(worker, num_items, self.num_workers, unroll=1):
                ...
                for i in cutlass.range(n_tiles, unroll=1):
                    ...
                    _wgmma(
                        tiled_mma_qk, acc_S,
                        tSrK[None, None, None, state_k.index], tSrQ, zero_init=True,
                    )
                    pipeline_k.consumer_release(state_k)
                    acc_S_mn = _acc_mn(acc_S)

                    # ---- online softmax over kv columns (per q_head) ----
    ...
    def combine_kernel(
        ...
        for idx in cutlass.range(tidx, self.num_splits, self.HEAD_DIM):
...
def attention_decode(
    ...
    num_splits=39, block_n=128, num_stages=3, num_producer_warps=1,
    num_workers=None, regs_producer=0, regs_consumer=0, fused=False,
    skip_combine=False, use_pdl=False,
):
    ...
    key = (
        ...
        num_splits, block_n, num_stages, num_producer_warps, num_workers,
        regs_producer, regs_consumer, fused, skip_combine, use_pdl, float(sm_scale),
    )
    ...
    with torch.cuda.device(q.device):
        ...
        if entry is None:
            kernel = FlashDecodeKernel(
                ...
                num_producer_warps=num_producer_warps, num_workers=num_workers,
                regs_producer=regs_producer, regs_consumer=regs_consumer,
                fused=fused, skip_combine=skip_combine, use_pdl=use_pdl, sm_scale=sm_scale,
            )
            o_part = torch.empty(
...
def attention(q, k, v, causal=True, sm_scale=None):
    ...
    (partial + combine). Sustains ~3.38 TB/s effective KV bandwidth.
    """
    if q.shape[2] == 1:
        return attention_decode(
            q, k, v, sm_scale=sm_scale, causal=False,
            num_splits=39, block_n=64, num_stages=5,
            num_producer_warps=2, fused=False,
        )
