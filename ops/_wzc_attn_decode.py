...
def attention_decode(q, k, v, sm_scale=None, causal=False, num_splits=NUM_SPLITS, block_n=BLOCK_N, num_stages=NUM_STAGES, num_workers=None):
    ...
    key = (
        ...
        num_splits, block_n, num_stages, num_workers, float(sm_scale),
    )
    ...
    with torch.cuda.device(q.device):
        ...
        if entry is None:
            ...
            kernel = FlashDecodeKernel(
                ...
                sm_scale=sm_scale, num_workers=num_workers,
            )
            o_part = torch.empty(
...
if __name__ == "__main__":
    ...
    o = attention_decode(q, k, v, num_splits=args.num_splits, block_n=args.block_n, num_stages=args.stages, num_workers=args.num_workers)
    ...
    for _ in range(args.warmup):
        attention_decode(q, k, v, num_splits=args.num_splits, block_n=args.block_n, num_stages=args.stages, num_workers=args.num_workers)
