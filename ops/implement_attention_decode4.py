"""Aggressively tuned fixed-128K Qwen3 Decode attention for H20.

Decode4 keeps Decode3's transposed WGMMA/TMA architecture and high-accuracy
FP16 K/V cache, then applies Hopper warpgroup register reallocation: the TMA
producer releases registers while the four WGMMA consumer warps receive a
larger accumulator budget.  This reduces register pressure/spills in the
partial kernel without changing its numerical dataflow or public contract.
...
# Decode4-only Hopper register-budget candidates.  The baseline dataflow and
# launch geometry stay identical so tuning isolates register pressure.
for _load_regs, _mma_regs in ((24, 208), (24, 216), (24, 224), (24, 240), (24, 248)):
    CONFIGS[f"decode4-r{_load_regs}-{_mma_regs}"] = {
        **CONFIGS["fp16-s8-n256-st1-w64-t160"],
        "num_regs_load": _load_regs,
        "num_regs_mma": _mma_regs,
    }
AUTO_CONFIG = "decode4-r24-224"
...
class FlashDecodeKernel:
    ...
    def __init__(
        ...
        alias_kv: bool = False,
        num_regs_load: int = 24,
        num_regs_mma: int = 232,
        ...
        with _COMPILE_LOCK:
            ...
            if compiled is None:
                kernel = FlashDecodeKernel(
                    ...
                    num_regs_load=values.get("num_regs_load", 24),
                    num_regs_mma=values.get("num_regs_mma", 232),
                    sm_scale=scale,
                )
