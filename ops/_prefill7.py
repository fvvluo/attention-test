...
# ---- 让依赖可被 import（本仓库相对路径，非机器绝对路径） ----
#   1) helpers.fmha_helpers 住在 baseline 的 CuTeDSL 示例目录下；把该目录
#      (含 helpers/ 包) 放到 sys.path，才能 `import helpers.fmha_helpers`。
#   2) 底层内核在同目录的 _fmha_prefill_kernel.py（下划线前缀，不被算子扫描器
#      当作算子导入）；把 ops/ 目录放到 sys.path 才能顶层 import 它。
_THIS_DIR = os.path.dirname(os.path.abspath(__file__))
_REPO_ROOT = os.path.dirname(_THIS_DIR)
_CUTEDSL = os.path.join(
    _REPO_ROOT, "flash-attention-baseline", "csrc", "cutlass",
    "examples", "python", "CuTeDSL",
)
for _p in (_CUTEDSL, _THIS_DIR):
    if os.path.isdir(_p) and _p not in sys.path:
        sys.path.insert(0, _p)

import helpers.fmha_helpers as fmha_utils  # noqa: E402
from _fmha_prefill_kernel import HopperFusedMultiHeadAttentionForward  # noqa: E402
...
class _PrefillPlan:
    ...
    def __init__(
        ...
        self.b, self.s_q, self.s_k = b, s_q, s_k
    ...
    def _prep(self, q, k, v, o, lse):
        ...
        """
        b, h_k, h_r, d = self.b, self.h_k, self.h_r, self.d

        # Q / O / LSE carry the full H = H_k * H_r query heads.
        qk = q.reshape(b, self.s_q, h_k, h_r, d).permute(1, 4, 3, 2, 0)
        ok = o.reshape(b, self.s_q, h_k, h_r, d).permute(1, 4, 3, 2, 0)
        lk = lse.reshape(b, self.s_q, h_k, h_r, 1).permute(1, 4, 3, 2, 0)
        # K carries only H_k KV heads (H_r broadcast handled inside the kernel).
        kk = k.reshape(b, self.s_k, h_k, 1, d).permute(1, 4, 3, 2, 0)
        ...
        if self.is_fp8:
            ...
            v_t = v.reshape(b, self.s_k, h_k, 1, d).permute(0, 4, 2, 3, 1).contiguous()
            vv = v_t.permute(4, 1, 3, 2, 0)
            v_leading = 0
        else:
            vv = v.reshape(b, self.s_k, h_k, 1, d).permute(1, 4, 3, 2, 0)
            v_leading = 1
    ...
    def __call__(self, q, k, v):
        b, s_q, s_k, h, d = self.b, self.s_q, self.s_k, self.h, self.d
...
def _get_plan(b, s_q, s_k, h, h_k, d, in_dtype_key, out_dtype_key, causal, sm_scale,
              tuning_key=None):
    ...
    return _PrefillPlan(
        b, s_q, s_k, h, h_k, d, in_dtype, out_dtype, causal, sm_scale, tuning
    )
...
def prefill_attention(
    ...
    plan = _get_plan(
        b, s_q, s_k, h, h_k, d, q.dtype, out_dtype, bool(causal), float(sm_scale),
        tuning_key,
    )
    o, lse = plan(q, k, v)
    if return_lse:
